...
CKPT_DIR = os.path.join(DATA_DIR, "sim49_checkpoints")
...
# (f_saw, eps0)
RUNS = [(f, 3e-4) for f in [6.04e9, 6.06e9, 6.08e9, 6.10e9, 6.12e9]]


def run_one(f_saw, eps0):
    tag = f"f{f_saw/1e9:.2f}_eps{eps0:.0e}"
    cp = os.path.join(CKPT_DIR, f"sim49_{tag}.npz")
...
def main():
    print("=" * 74, flush=True)
    print("Sim 49: FINE f_SAW scan @ eps0=3e-4 (20MHz steps, catch narrow tongue) (realistic YIG, B0=50mT)", flush=True)
    print("=" * 74, flush=True)
